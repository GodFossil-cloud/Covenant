...
- `assets/footer-overrides.css` — head-loaded footer geometry + tone overrides (Lexicon flat-top seal + small contrast nudges, including citation label styling beneath the Lexicon seal; also consolidates retired Lexicon quiet-open seal/glyph overrides, locked-state feedback, glyph state-color rules, tap-flash timing rules, dock-tab tap-flash rules, and the Lexicon header/quote-box alignment rules that had lived in separate micro-files, to reduce asset sprawl).
