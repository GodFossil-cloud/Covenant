...
- `assets/footer-overrides.css` — head-loaded footer geometry + tone overrides (Lexicon flat-top seal + small contrast nudges, including citation label styling beneath the Lexicon seal; also consolidates retired Lexicon quiet-open seal/glyph overrides, locked-state feedback, glyph state-color rules, and tap-flash timing rules to reduce asset sprawl).
